...
public class nhanvienimpl implements nhanvienDAO {
    ...
    public nhanvien create(nhanvien entity) {
        ...
        try {
            Object[] values = {
                ...
                entity.getSdt(),
                entity.getGioiTinh(),
                parseToSqlDate(entity.getNgaySinh()),
                entity.getDiaChi(),
                entity.getTrangThai(),
                entity.getMaPhong(),
                entity.getMaChucVu(),
    ...
    private Date parseToSqlDate(String ngaySinhText) {
        if (ngaySinhText == null) return null;
        String text = ngaySinhText.trim();
        ...
        for (DateTimeFormatter f : fmts) {
            try {
                ld = LocalDate.parse(text, f);
                break;
            } catch (DateTimeParseException ignore) { }
        }
        if (ld == null) {
            throw new RuntimeException("Định dạng ngày sinh không hợp lệ: " + text + ". Vui lòng dùng dd-MM-yyyy hoặc dd/MM/yyyy hoặc yyyy-MM-dd");
        }
        ...
        try (
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    nv.setSdt(rs.getString("SDT"));
                    nv.setGioiTinh(rs.getString("GioiTinh"));
                    // Đọc NgaySinh thành String để khớp kiểu của entity
                    nv.setNgaySinh(rs.getString("NgaySinh"));
                    nv.setDiaChi(rs.getString("DiaChi"));
                    nv.setNgayVaoLam(rs.getDate("NgayVaoLam"));
                    nv.setTrangThai(rs.getString("TrangThai"));
            ...
            e.printStackTrace();
            throw new RuntimeException("Lỗi khi tìm nhân viên theo mã: " + e.getMessage());
        }
        return null;
    }
